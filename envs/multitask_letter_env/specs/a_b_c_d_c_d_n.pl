...
trace_expression('Main', Main) :-
    ...
    S0_B = gen(['n'], (star((not_abcd:eps)) * ((b_match:eps) * app(S1_C, [var('n')])))),
    S1_C = gen(['n'], (star((not_abcd:eps)) * ((c_match:eps) * app(S2_D, [var('n')])))),
    S2_D = gen(['n'], (star((not_abcd:eps)) * ((d_match:eps) * app(S3_C, [var('n')])))),
    S3_C = gen(['n'], (star((not_abcd:eps)) * ((c_match:eps) * app(S4_D, [var('n')])))),
    S4_D = gen(['n'], guarded((var('n') > 0), (star((not_abcd:eps)) * ((d_match:eps) * app(S4_D, [(var('n') - 1)]))), 1)).
