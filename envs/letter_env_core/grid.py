"""Shared gridworld mechanics for LetterEnv variants."""
...
class LetterAction(IntEnum):
    """Actions used by LetterEnv-family gridworlds."""
...
class LetterGridWorld(gym.Env):
    """Grid, movement, proposition, and replacement mechanics."""
    ...
    def reset_grid(self, *, sampled_n: int) -> None:
        self.sampled_n = int(sampled_n)
    ...
    def move_agent(self, action: int) -> str:
        action = int(action)
        ...
        if label != NO_PROPOSITION:
            self.record_observation(label)
        return label

    def make_observation(self, observed_label: str | None = None) -> dict[str, np.ndarray | int]:
    ...
    def record_observation(self, label: str) -> None:
        self.proposition_observation_counts[label] = (
            self.proposition_observation_counts.get(label, 0) + 1
        )

    def apply_replacement_if_needed(self, label: str) -> None:
