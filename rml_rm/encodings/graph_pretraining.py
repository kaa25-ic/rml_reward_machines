...
def train_gnn_dynamics(config: GNNDynamicsConfig) -> dict[str, Any]:
    ...
    examples, event_vocab, phase_vocab = load_monitor_corpus_examples(config.dataset_path)
...
def load_monitor_corpus_examples(dataset_path: Path) -> tuple[list[DynamicsExample], dict[str, int], dict[str, int]]:
    ...
    for row in sorted(rows, key=_row_sort_key):
        ...
        previous_state = normalize_generated_variables(
            str(previous_row.get("normalized_monitor_state") or previous_row["monitor_state"])
        )
        next_state = normalize_generated_variables(
            str(row.get("normalized_monitor_state") or row["monitor_state"])
        )
...
def _trace_key(row: dict[str, Any]) -> tuple[Any, ...]:
    return (
        row.get("task_key", row.get("task_id")),
        row.get("trace_index"),
        row.get("trace_type"),
        row.get("n"),
    )
...
def _transition_label(previous_state: str, next_state: str, row: dict[str, Any]) -> int:
    ...
    if next_state == "false_verdict" or verdict == "false" or bool(row.get("failure", False)):
